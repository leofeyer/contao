		...
		for ($_rit=0, $_cnt=\count($tags); $_rit<$_cnt; $_rit+=2)
		{
			...
			switch (strtolower($elements[0]))
			{
				...
				case 'link_name':
					...
					{
						case 'link':
							$arrCache[$strTag] = sprintf('<a href="%s" title="%s"%s%s>%s</a>', $strUrl ?: ($request?->getBasePath() ?? '') . '/', StringUtil::specialcharsAttribute($strTitle), $strClass, $strTarget, $strName);
							break;

						case 'link_open':
							$arrCache[$strTag] = sprintf('<a href="%s" title="%s"%s%s>', $strUrl ?: ($request?->getBasePath() ?? '') . '/', StringUtil::specialcharsAttribute($strTitle), $strClass, $strTarget);
							break;

						case 'link_url':
							$arrCache[$strTag] = $strUrl ?: ($request?->getBasePath() ?? '') . '/';
							break;
